...
async def timediff(a):
	global pretime
	seconds = time.time() - pretime
...
async def coin_sys(call: types.CallbackQuery):
	if call.data == "1" or "2":
				...
				if call.data == "1":
					...
					use = "Решка"
				if call.data == win1:
					message.edit_message_text(chat_id=call.message.chat.id, message_id=mess, text ="@" + str(pon)+ " Выпал " + str(lose) + " вы выиграли\nВаша ставка: " + str(use))
					await message.answer_callback_query(callback_query_id=call.id, text='А вы сегодня везучий!')
					wait = 0
					return wait
				else:
				   bot.edit_message_text(chat_id=call.message.chat.id, message_id=mess, text ="@" + str(pon) +" Выпал " + str(lose) + " вы проиграли\nВаша ставка: " + str(use))
...
async def start (message: types.Message):
	...
	if date in morning:
		...
		await message.answer('👋 • Добрый день, @' + name+'.\n\n🧑🏻‍💼 • Я вспомогательный бот для Fukk Sleep Chat. Пиши команду ниже, что бы узнать список моих команд.\n\n/support_help')
	elif date in subnight:
		await message.answer('👋 • Добрый вечер, @' + name+'.\n\n🧑🏻‍💼 • Я вспомогательный бот для Fukk Sleep Chat. Пиши команду ниже, что бы узнать список моих команд.\n\n/support_help')
	elif date in night:
		await message.answer('👋 • Доброй ночи, @' + name+'.\n\n🧑🏻‍💼 • Я вспомогательный бот для Fukk Sleep Chat. Пиши команду ниже, что бы узнать список моих команд.\n\n/support_help')
	else:
		await message.answer('👋 • Здравствуйте, @' + name+'.\n\n🧑🏻‍💼 • Я вспомогательный бот для Fukk Sleep Chat. Пиши команду ниже, что бы узнать список моих команд.\n\n/support_help')
