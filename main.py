...
ptime = time.time()
def timediff(a):
    seconds = time.time() - previous_time
    previous_time = time.time()
    if float(f'{seconds:.1f}') > a:
        return True
    else: 
        return False
@bot.message_handler(commands=['botinf'])
def binf(message):
...
def coin(message):
    ...
    else: bot.send_message(message.chat.id, 'Эу даун щас кулдаун')
...
def coin_sys(call):
    if call.data == "1" or "2":
                ...
                if call.data == "1":
                    ...
                    use = "Решка"
                if call.data == win1:
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=mess, text ="@" + str(pon)+ " Выпал " + str(lose) + " вы выиграли\nВаша ставка: " + str(use))
